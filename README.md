...
- **Reads like a real review** — the posted comment opens with a plain-language summary naming what actually matters, then findings grouped by file the way a person reads a diff — not a dimension-by-dimension table with emoji severity badges. Code suggestions are still syntax-highlighted and language-tagged (```` ```js ```` / ```` ```py ````).
