...
// Builds a short "why this file changed" note per file by collecting the
// review findings (issues + recommendations) attached to that file, so the
// generated PR description explains each change instead of just naming it.
function notesForFile(review, fileName) {
  const notes = [];
  for (const cat of CATEGORIES) {
    for (const item of review[cat.key] || []) {
      ...
      const cleanIssue = item.issue
        ? item.issue.replace(new RegExp(`\\s+in\\s+${escapeRegex(file)}$`), '').trim()
        : '';
...
function missingTestWarning(review) {
  for (const item of review.best_practices || []) {
    ...
    if (/no test file|missing[- ]test|touches no test/i.test(text)) {
...
function generateDescription(review) {
  ...
  for (const f of files || []) {
    ...
    const notes = notesForFile(review, f.name);
    if (notes.length) {
      md += `  - _Why:_ ${notes.join(' ')}\n`;
    }
  }
  ...
  if (testFiles.length) {
    ...
    for (const tf of testFiles) md += `- \`${tf}\`\n`;
  } else if (missingTest) {
    md += `⚠️ **No test files were changed.** ${missingTest}\n`;
  } else {
  ...
  md += `\n## ✅ Test Case Execution\n\n`;
  if (testFiles.length) {
    md += `Verify the change by running the suite for the touched code. Checklist:\n`;
    for (const tf of testFiles) md += `- [ ] \`${tf}\` passes\n`;
    md += `- [ ] Full test suite is green\n`;
  } else {
    md += `No automated tests were added. Recommended before merge:\n`;
    md += `- [ ] Add a test covering the changed behavior\n`;
    md += `- [ ] Manually verify the affected flow\n`;
  }
  ...
  md += `- **Verdict** — ${verdictBadge(recommendation.verdict)}\n`;
