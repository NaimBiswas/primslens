...
function buildOpeningSummary(review, findings, meta) {
  ...
  if (findings.length === 0) {
    return `Went through the ${fileCount} changed ${fileWord} — nothing to flag.`;
  }

  const actionable = findings.filter((f) => f.type === 'BUG' || f.type === 'CONCERN');
  if (actionable.length === 0) {
    return `Went through the ${fileCount} changed ${fileWord} — nothing blocking, just a few notes below.`;
  }
...
function generateReviewBody(review) {
  ...
  if (recommendation.verdict === 'APPROVE' && !hasAnyFindings(review)) {
    ...
    return [
      `**${meta.prTitle}**`,
      '',
      `Went through the ${fileCount} changed file${fileCount === 1 ? '' : 's'} — nothing to flag. Approve.`,
      '',
      '_— PrismLens_',
    ].join('\n');
  }
