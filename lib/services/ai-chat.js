...
const TIMEOUT_REPLY = 'Reached the tool-call limit without a final answer — try rephrasing or breaking the request into smaller steps.';
...
async function runAnthropicChat(apiKey, model, systemPrompt, history, message, execTool) {
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    ...
    if (!toolUses.length) return { content: text, committed };

    messages.push({ role: 'assistant', content: blocks });
    const toolResults = [];
    for (const use of toolUses) {
      if (use.name === 'commit_file') committed = true;
...
async function runGeminiChat(apiKey, model, systemPrompt, history, message, execTool) {
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round++) {
    ...
    if (!functionCalls.length) return { content: text, committed };

    contents.push({ role: 'model', parts });
    const responseParts = [];
    for (const p of functionCalls) {
      if (p.functionCall.name === 'commit_file') committed = true;
 ...
 * Returns `{ content, committed }` — `committed` is true only if the model
 * actually called `commit_file` this turn, so callers (automation's
 * pending-approval tracking) know a fix was really pushed, not just proposed.
 */
