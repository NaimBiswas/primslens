...
function withActivityFooter(body, activityId) {
  ...
  return `${body}\n\n<sub>PrismLens automation · activity number #${activityId}</sub>`;
}

// GitHub marks bot-owned accounts with `type: "Bot"` on the user object —
// covers vercel[bot], github-actions[bot], dependabot[bot], etc. The
// `[bot]`-suffix login check is a fallback for the rare payload that omits
// `type` but still uses the naming convention every GitHub bot account has.
export function isFromBot(comment) {
...
export async function processAutomatedComment({ installationId, eventType, payload, deliveryId, activityId }) {
  ...
  const { githubToken: token, aiOverride } = installation;
  ...
  // Only react to people — a comment from vercel[bot], github-actions[bot],
  // dependabot[bot], etc. isn't someone to reply to or confirm a fix with.
  // Our own replies are posted under the account owner's real user token
  // (not a separate bot identity), so this can't accidentally match them —
  // that's what the isBotComment check above is for.
  if (isFromBot(comment)) {
    await recordActivity(installationId, { eventType, deliveryId, outcome: 'skipped', reason: 'comment from a bot account' });
    return { skipped: 'comment from a bot account' };
  }
  ...
  try {
    ...
    const review = await analyzePR(pr, files, config, token, undefined, aiOverride);
...
export async function processAutomatedPullRequest({ installationId, payload, deliveryId, activityId }) {
  ...
  try {
    ...
    const review = await analyzePR(prData, files, config, token, undefined, aiOverride);
...
/**
 * Points this installation's automation at a specific provider/key — pass
 * null (or an object missing providerId/apiKey) to clear it and revert to
 * the server's env-configured default.
 */
export async function setAutomationAIConfig(installationId, config) {
  const installation = await getInstallation(installationId);
  if (!installation) throw new Error('Unknown installation');
  await setInstallationAIConfig(installationId, config);
}
...
export async function getInstallationStatus(installationId) {
  ...
  return {
    ...
    botLogin,
    // Never the key itself — just enough for the dashboard to show what
    // automation is actually using.
    aiProviderId: installation.aiOverride?.providerId || null,
    aiModel: installation.aiOverride?.model || null,
    recentActivity,
    pendingApprovals,
  };
}
