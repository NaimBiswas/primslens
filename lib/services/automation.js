...
/**
 * Reacts to a single GitHub webhook comment event for one installation:
 * decides whether it's relevant (assigned to / authored by that
 * installation's own token owner, not our own reply), runs it through the
 * same chat agent the interactive UI uses, and posts the reply back to the
 * right place. Propose-only — never commits.
 */
