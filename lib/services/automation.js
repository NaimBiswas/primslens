 ...
 */
export async function processAutomatedComment({ installationId, eventType, payload, deliveryId }) {
 ...
 */
export async function processAutomatedPullRequest({ installationId, payload, deliveryId }) {
  const installation = await getInstallation(installationId);
  if (!installation) return { skipped: 'unknown installation' };
  const { githubToken: token } = installation;
