...
export async function processAutomatedComment({ installationId, eventType, payload }) {
  ...
  if (await isBotComment(installationId, comment.id)) return { skipped: 'own comment' };
  ...
  try {
    ...
    if (eventType === 'pull_request_review_comment') {
      posted = await replyToReviewComment(prUrl, token, comment.id, result.content);
    } else {
      posted = await postIssueComment(prUrl, token, result.content);
    }
 ...
 */
export async function processAutomatedPullRequest({ installationId, payload }) {
  const installation = await getInstallation(installationId);
  if (!installation) return { skipped: 'unknown installation' };
  const { githubToken: token } = installation;
  ...
  const base = { prUrl, eventType: 'pull_request' };
  ...
  try {
    ...
    const posted = await postPRReview(prUrl, token, review, 'COMMENT');
