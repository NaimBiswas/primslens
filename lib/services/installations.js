...
function ensureSchema() {
  if (!schemaReady) {
    schemaReady = (async () => {
      ...
      // When unset, automation falls back to whatever provider the server's
      // own env vars resolve to (see resolveEnvBackend in ai-providers.js) —
      // this is what lets an account instead point automation at the same
      // provider/key it already uses interactively via the Model page.
      await sql`ALTER TABLE automation_installations ADD COLUMN IF NOT EXISTS ai_provider_id TEXT`;
      await sql`ALTER TABLE automation_installations ADD COLUMN IF NOT EXISTS ai_api_key_encrypted TEXT`;
      await sql`ALTER TABLE automation_installations ADD COLUMN IF NOT EXISTS ai_model TEXT`;
      await sql`
...
function rowToInstallation(row) {
  return {
    ...
    // null unless the account explicitly pointed automation at a specific
    // provider/key (see setInstallationAIConfig) — automation.js falls back
    // to the server's env-configured provider when this is null.
    aiOverride: row.ai_provider_id && row.ai_api_key_encrypted
      ? { providerId: row.ai_provider_id, apiKey: decryptSecret(row.ai_api_key_encrypted), model: row.ai_model || undefined }
      : null,
...
/**
 * Points this installation's automation at a specific provider/key instead
 * of the server's env-configured default — typically the same provider/key
 * the account already has active on the Model page, so automation stops
 * defaulting to whichever provider the server happens to have an env key
 * for first (see resolveEnvBackend in ai-providers.js). Passing a falsy
 * `config` (or one missing providerId/apiKey) clears the override, reverting
 * this installation back to the server default.
 */
export async function setInstallationAIConfig(id, config) {
  await ensureSchema();
  if (!config?.providerId || !config?.apiKey) {
    await sql`UPDATE automation_installations SET ai_provider_id = NULL, ai_api_key_encrypted = NULL, ai_model = NULL WHERE id = ${id}`;
    return;
  }
  await sql`
    UPDATE automation_installations
    SET ai_provider_id = ${config.providerId}, ai_api_key_encrypted = ${encryptSecret(config.apiKey)}, ai_model = ${config.model || null}
    WHERE id = ${id}
  `;
}
 ...
 *
 * Returns the row's own id — the initial "received" call's id is threaded
 * through to processAutomatedComment/processAutomatedPullRequest so the
 * reply/review actually posted to GitHub can cite it, letting a GitHub
 * comment be matched back to its dashboard row.
 */
...
export async function listActivity(installationId) {
  await ensureSchema();
  const rows = await sql`
    SELECT id, at, outcome, event_type, pr_url, pr_title, reason, delivery_id FROM automation_activity
