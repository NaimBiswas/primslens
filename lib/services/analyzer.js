...
function buildRecommendation(allReviews) {
  ...
  if (criticalBugs > 0) return { verdict: 'REJECT', label: 'Reject or Rework', reason: `${criticalBugs} critical security issue${criticalBugs === 1 ? '' : 's'} need${criticalBugs === 1 ? 's' : ''} fixing` };
  if (highBugs > 0) return { verdict: 'REVIEW', label: 'Review Required', reason: `Fix ${highBugs} high-severity bug${highBugs === 1 ? '' : 's'} before merging` };
  if (concerns > 3) return { verdict: 'REVIEW', label: 'Review Required', reason: `Address ${concerns} concerns before merging` };
...
function buildResult(prData, files, allReviews, analysisMode, fallbackReason, aiProvider, ticketDescription) {
  ...
  return {
    meta: {
      ...
      // True only when this review was scoped to a pasted ticket description
      // (see buildTicketReviewPrompt in ai-review.js) rather than the
      // standard 6-dimension pass — lets the UI/CLI say what was actually
      // checked instead of implying a generic review ran.
      ticketValidated: !!ticketDescription,
      // The pasted ticket text itself (already capped in buildReviewContext),
      // carried through the result so a later Describe call — which only
      // receives this review object, not the original request — can base
      // the generated PR description on the same ticket instead of just the
      // diff stats.
...
const MAX_TICKET_DESCRIPTION_CHARS = 6000;
...
async function buildReviewContext(prData, analyzableFiles, token, onProgress, ticketDescription) {
  ...
  return {
    prTitle: prData.title,
    // Present only when the caller wants the review scoped to a ticket
    // (see analyzePR's `ticketDescription` param) — streamReviewFindings
    // switches prompts based on this field alone.
    ticketDescription: ticketDescription
      ? String(ticketDescription).trim().slice(0, MAX_TICKET_DESCRIPTION_CHARS)
      : null,
...
async function analyzeWithAI(prData, files, { providerId, apiKey, model }, config, token, onProgress, ticketDescription) {
  const analyzableFiles = files.filter((f) => !isIgnored(f.filename, config?.ignorePaths));
  const context = await buildReviewContext(prData, analyzableFiles, token, onProgress, ticketDescription);
  const provider = getProvider(providerId);

  emit(onProgress, 'ai-review', context.ticketDescription
    ? `Validating ${context.files.length} changed file(s) against the ticket with ${provider.name}…`
    : `Reviewing ${context.files.length} changed file(s) with ${provider.name}…`);
  ...
  // The missing-test check is a generic code-quality reflex, not a ticket
  // requirement — leave it out of a ticket-scoped review so every finding
  // traces back to the ticket, per the ticket-review prompt's contract.
  const allReviews = applyReviewConfig(
    context.ticketDescription ? findings : [...findings, ...checkMissingTests(analyzableFiles)],
    config
  );
  return buildResult(prData, files, allReviews, 'ai', null, provider.name, context.ticketDescription);
}
 ...
 *
 * `ticketDescription`, when non-empty, scopes the whole review to "does
 * this PR satisfy this ticket" (see buildTicketReviewPrompt in
 * ai-review.js) instead of the standard 6-dimension pass. That judgment
 * requires an actual model — the regex fallback can't read prose — so
 * unlike the normal AI-unavailable path, a ticket-scoped review with no
 * usable AI backend fails outright rather than silently returning a
 * generic pattern-based review the caller didn't ask for.
 */
export async function analyzePR(prData, files, config, token, onProgress, aiOverride, ticketDescription) {
  ...
  if (ticketDescription && !backend) {
    throw new Error('Ticket-based review requires an AI backend — connect a provider key on the Model page');
  }
  ...
  if (backend) {
    try {
      result = await analyzeWithAI(prData, files, backend, config, token, onProgress, ticketDescription);
    } catch (err) {
      if (ticketDescription) throw err; // no meaningful ticket-scoped fallback — surface the failure instead of a silent generic review
