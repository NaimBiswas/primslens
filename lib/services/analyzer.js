...
async function analyzeWithOpenCode(prData, files, opencodePath, config, token) {
  ...
  try {
    const stdout = await new Promise((resolve, reject) => {
      ...
      args.push(
        ...
        '--log-level', 'ERROR',
        `You MUST return ONLY raw JSON. Read .prismlens-review-context.json and analyze ${context.files.length} file(s) using their full content and repoTree for codebase context, not just the patch. Output: {"findings":[...]}`,
      );
      ...
      const timer = setTimeout(() => { child.kill(); reject(new Error('Timed out after 240s')); }, 240000);
    ...
    const allReviews = applyReviewConfig([
      ...parsed.findings.map((f) => ({ ...f, file: f.filename || '' })),
...
function analyzeFallback(prData, files, config) {
  const analyzableFiles = files.filter((f) => !isIgnored(f.filename, config?.ignorePaths));
  const allReviews = applyReviewConfig(
 ...
 */
export async function analyzePR(prData, files, config, token) {
  const opencodePath = await findOpenCode();
  let result;
  if (opencodePath) {
    try {
      result = await analyzeWithOpenCode(prData, files, opencodePath, config, token);
    } catch (err) {
      console.warn('OpenCode analysis failed, using fallback:', err.message);
    }
  }
  if (!result) result = analyzeFallback(prData, files, config);
