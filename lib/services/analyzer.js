...
async function analyzeWithOpenCode(prData, files, opencodePath, config, token, onProgress) {
  ...
  try {
    const stdout = await new Promise((resolve, reject) => {
      ...
      const scanner = createJsonValueScanner((value) => {
        if (looksLikeFinding(value)) {
          const finding = { ...value, file: value.filename || '' };
          ...
          wrapperFindings = value.findings.map((f) => ({ ...f, file: f.filename || '' }));
        } else if (Array.isArray(value)) {
          wrapperFindings = value.map((f) => ({ ...f, file: f.filename || '' }));
        }
      });
    ...
    if (!findings) {
      ...
      findings = parsed.findings.map((f) => ({ ...f, file: f.filename || '' }));
    }
 ...
 */
export async function analyzePR(prData, files, config, token, onProgress) {
  const opencodePath = await findOpenCode();
  let result;
  let fallbackReason;
  if (!opencodePath) {
    fallbackReason = 'opencode CLI not found';
  } else {
    try {
