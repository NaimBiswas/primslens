 ...
 */
export async function reviewPR(prUrl, token, onProgress, geminiOverride) {
  const res = await fetch(`${API_BASE}/review`, {
    ...
    body: JSON.stringify({
      prUrl,
      token,
      geminiApiKey: geminiOverride?.apiKey,
      geminiModel: geminiOverride?.model,
      geminiDisabled: geminiOverride?.disabled,
    }),
  });
