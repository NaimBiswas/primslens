import { resolveAIOverride } from './ai-local.js';
 ...
 *
 * `ticketDescription`, if given, scopes the whole review to whether the PR
 * satisfies that ticket instead of the standard 6-dimension pass — see
 * analyzePR's doc comment in lib/services/analyzer.js. Requires an AI
 * backend to be configured/connected; the request fails otherwise.
 */
export async function reviewPR(prUrl, token, onProgress, ticketDescription) {
  const aiOverride = resolveAIOverride();
  const res = await fetch(`${API_BASE}/review`, {
    ...
    body: JSON.stringify({
      ...
      aiProviderId: aiOverride?.providerId,
      aiApiKey: aiOverride?.apiKey,
      aiModel: aiOverride?.model,
    }),
  });
