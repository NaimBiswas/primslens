...
export function skipReason(eventType, payload) {
  ...
  if (eventType === 'pull_request_review') {
    if (payload.action !== 'submitted') return 'not a submitted action';
    if (!payload.review?.body?.trim()) return 'review has no body text';
    return null;
  }
  if (eventType === 'pull_request') {
    if (payload.action !== 'opened') return 'not an opened action';
    return null;
  }
