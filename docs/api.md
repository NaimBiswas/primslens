...
A `200` with `{ "skipped": "<reason>" }` means the event was received but wasn't relevant (not an `opened`/`created`/`submitted` action, a `pull_request_review` with no body text, a comment posted by a bot account (`vercel[bot]`, `github-actions[bot]`, `dependabot[bot]`, etc. — GitHub's own `type: "Bot"` marker), or the PR isn't assigned to/authored by the account's own login) — this is normal, not an error.
