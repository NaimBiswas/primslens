...
Both are written into `.prismlens-review-context.json` as `files[].fullContent` and `repoTree`/`repoTreeTruncated`, and `.opencode/agents/prismlens-review.md` instructs the agent to read `fullContent` as its primary material while still anchoring findings to lines the PR actually changed (never flagging pre-existing code it happens to see while reading context). The regex fallback path is unaffected — it's inherently diff-based (it only ever looks at added lines) and has no model to reason about a wider codebase.

`prismlens-review`'s agent contract streams one finding object per line as the model produces it, rather than one `{"findings": [...]}` blob at the end — `analyzeWithOpenCode()` parses these incrementally via `json-value-stream.js`'s `createJsonValueScanner()`, which recognizes a complete top-level JSON value the moment it closes regardless of chunking, and still falls back to parsing one whole-blob value if a model ignores the streaming instruction. The per-review opencode timeout was raised repeatedly (180s → 240s → now 1200s, matching `chat.js`'s budget for the same opencode-backed call — see PRODUCT.md's "chat replies can take up to ~20 minutes per turn") after the larger codebase-aware context kept pushing real PRs past each successive cutoff, silently discarding the AI's work to the regex fallback every time. Because findings are now parsed as they stream in rather than only once the process exits cleanly, a timeout is no longer a full loss either: whatever findings were already streamed back are kept and used, and only a timeout with zero findings collected falls all the way back to the regex analyzer.

**Run-to-run consistency.** Two sources of variance are worth knowing about. First, `prismlens-review`'s agent frontmatter pins `temperature: 0` so the model samples as deterministically as the provider allows — this narrows but can't fully eliminate variation between runs of the same PR, since not every provider/model honors `temperature`. Second, `analyzePR()` silently swallowed the AI path's failure reason before falling back to the regex analyzer, so a transient opencode failure (timeout, bad exit code, unparseable output, missing binary) would land the user on a structurally different finding set — different rules, different categories, different severities — with no visible signal beyond a small `mode-badge`. `analyzePR()` now threads the actual failure reason into `result.meta.fallbackReason`, and `CodeReviewPanel.jsx` renders a banner above the report ("AI review unavailable for this run…") whenever `analysisMode === 'fallback'`, naming the reason so a fallback-mode result isn't mistaken for an inconsistent AI review.
